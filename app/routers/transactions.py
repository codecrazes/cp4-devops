...
def post_transaction(payload: TransactionCreate, db: Session):
    return create_transaction(db, payload)


@router.put("/{tx_id}", response_model=TransactionOut)
def put_transaction(tx_id: int, payload: TransactionUpdate, db: Session):
...
def del_transaction(tx_id: int, db: Session):
